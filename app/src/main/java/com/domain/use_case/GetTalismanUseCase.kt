...
class GetTalismanUseCase @Inject constructor(
    private val repository: TalismanRepository
) {
    @RequiresExtension(extension = Build.VERSION_CODES.S, version = 7)
    operator fun invoke(serverId: String, characterId: String): Flow<Resource<TalismanDto>> =
        flow {
            emit(Resource.Loading())
            try {
                // Fetch the response from the repository
                val response = repository.getTalisman(serverId, characterId, API_KEY)
