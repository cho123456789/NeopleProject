...
class GetBufferCreatureUseCase @Inject constructor(
    private val repository: BufferCreatureRepository
) {
    @RequiresExtension(extension = Build.VERSION_CODES.S, version = 7)
    operator fun invoke(serverId: String, characterId: String): Flow<Resource<BufferCreatureDto>> =
        flow {
            emit(Resource.Loading())
            try {
                // Fetch the response from the repository
                val response = repository.getBufferCreature(serverId, characterId, API_KEY)
