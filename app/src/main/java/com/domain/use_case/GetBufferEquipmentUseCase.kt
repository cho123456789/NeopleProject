...
class GetBufferEquipmentUseCase @Inject constructor(
    private val repository: BufferEquipmentRepository
) {
    @RequiresExtension(extension = Build.VERSION_CODES.S, version = 7)
    operator fun invoke(serverId: String, characterId: String): Flow<Resource<BufferEquipmentDto>> =
        flow {
            emit(Resource.Loading())
            try {
                // Fetch the response from the repository
                val response = repository.getBufferEquipment(serverId, characterId, API_KEY)
